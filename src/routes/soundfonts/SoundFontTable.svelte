			...
			<thead class="sticky-header bg-100">
				<tr>
					...
					{#if !addSoundFontButtonActive}
						<th class="text-right"
							><button class="btn btn-primary btn-xs" on:click={() => dispatch('showAddSoundFont')}
								>Add SoundFont</button
							></th
						>
					{:else}
						<th></th>
				...
				{#if soundfontList.length > 0}
					{#each soundfontList as soundfont, index}
						<tr class="hover">
							...
							<td class="capitalize">{soundfont.description}</td>
							<td><button class="hover:underline" on:click={() => dispatch('openEditSoundFont', soundfont.guid)}>Details</button></td>
						</tr>
					{/each}
				{/if}
			</tbody>
